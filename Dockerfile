...
RUN echo "deb http://ftp.debian.org/debian/ stretch main" >> /etc/apt/sources.list && \
    apt-get update && \
    apt-get install -y --no-install-recommends libtool gcc-5 g++-5 automake build-essential libboost-all-dev expat libcgal-dev libsparsehash-dev libcairomm-1.0-1
